...
public class horario extends Listacursos {
    ...
    private void nuevoHorario(int diastogo, int HMAX, int HMIN, int huecos, int[] diastogo2) {
        String code = "";
        curso actual = cabeza;
        while (actual != null) {
        ...
        System.out.println("HORARIO:  " + code);
        Hopcion nuevo = new Hopcion(code, diastogo, HMAX, HMIN, huecos, diastogo2);
        nuevo.favorites = this.loscursos.favorites();
        if (hhead == null) {
            hhead = nuevo;
            hact = nuevo;
        } else {
        ...
        if (cursoact == null) {
            ...
            {   
                ...
                if (diastogo <= diasmax && horasmax <= horamaxm && horasmin >= horaminm && huecosd <= huecomax && checkdias()) {
                    nuevoHorario(diastogo, horasmax, horasmin, huecosd, loscursos.diastogo());
                    cont++;
                }
            }
            return;
