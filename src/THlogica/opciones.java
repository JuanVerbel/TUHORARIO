...
//    }
    ...
    private boolean DNCdays(String code) { // (   abc,abc,   . )  DESENCRIPTA CADA DIA
        String[] componentes = code.split("\\,");
        if (componentes.length < ndias || componentes.length > ndias +1) {
        ...
        if (componentes.length == ndias +1 && !componentes[componentes.length-1].equalsIgnoreCase("none")){
            System.out.println("NOMBRE PROFE: " + componentes[componentes.length-1]);
            nu.Label = componentes[componentes.length-1];
            componentes[componentes.length-1] = "l";
        }
        
        
        for (int i = 0; i < componentes.length; i++) {
            if (!componentes[i].equals("l")) DNCsingle(componentes[i], nu);
        }
        newaction(nu,"added");
        return true;
    }
